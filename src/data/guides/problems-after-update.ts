  ...
  content: [
    {
      ...
      body: `Released ${latestPatch.releasedAt}: ${latestPatch.headline}. Close the game fully (not minimized). Exit Steam and reopen it. Confirm Meccha Chameleon shows no pending download. Relaunch from the Steam library — not a desktop shortcut to an old build. Have every friend repeat these steps. One outdated player breaks private lobbies even when passwords and server names are correct. After 4.1.0 join with the 5-character in-game code — the old server-name browser is gone. For late-August 4.0–4.1.x issues (Kyoto, Art Museum, join codes, Petit Chameleon, Greece/Egypt rework, crashes), see the v4.0–4.1 fix guide. For mid-August 3.7–3.9.x issues (Shopping Mall, Mo Eeyo, BAN, jump height, Deep Sea OOB, voice flash, emote ceiling gap), see the v3.7–3.9 fix guide. For early-August 3.6.x issues (Deep Sea, settings persist, crowd FPS, chat filter), see the v3.6 fix guide. For Exit 8 / friend search / voice from 3.4–3.5, see Problems After v3.4–3.5. For late-July 3.1–3.3 issues (Banban, Workshop malware panic, Discord scam links, eyedropper UI change, Plump), see the v3.1–3.3 fix guide. For Greece / glow paint from 3.0.0 / 2.9.0, see Problems After v3.0.0.`,
    },
    {
      id: "version-specific",
      heading: `${latestPatch.version} — Version-Specific Issues`,
      body: "Update 4.1.1 (September 16) adds voice-chat disable, touch-screen paint, a mic icon, incoming voice muted by default, and a temporary Switch 2 host cap of 10; crash is still under investigation. Switch 2 launched September 9 with Steam PC cross-play (Hotfix 4.1.0). Update 4.1.0 adds Kyoto and Art Museum, replaces the server browser with 5-character join codes, and reworks Greece and Egypt; 4.0.0 adds host-gated Petit Chameleon (x0.5). Full symptom walkthrough: Problems After v4.0–4.1 (/guides/problems-after-v4-1-0). Update 3.9.0 adds Shopping Mall and player IDs; 3.8.0 adds Mo Eeyo skip-votes and host BAN; 3.7.2 closes Deep Sea OOB (Steam title typo fix7.3.2); 3.9.1 fixes emote paging and ceiling-cling gaps. Full symptom walkthrough: Problems After v3.7–3.9 (/guides/problems-after-v3-9-0). Update 3.6.0 adds Deep Sea and settings persistence; fix3.6.1 cuts crowded-lobby FPS drops and softens over-strict chat filters: Problems After v3.6. Exit 8 / friend search / voice: Problems After v3.4–v3.5. Update 3.3.0 adds the Plump body type. 3.2.0 adds 12 emotes and removes the eyedropper button from the color palette (sampling via Spacebar / 3D eyedropper remains). 3.1.0 adds Garten of Banban, expands controller non-UI support, and ships the MOD-map security patch after Workshop malware reports. Greece / glow: Problems After v3.0.0. Death Burger / chat filter: Problems After v2.8.0. Egypt/HIKAKIN: Problems After v2.7.0. Osaka/paint: Problems After v2.5.0.",
    },
    {
      id: "lobby-broken",
