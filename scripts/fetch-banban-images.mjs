/**
 * Downloads Garten of Banban map hero + unique cropped spot thumbs.
 ...
 *
 * Run: node scripts/fetch-banban-images.mjs
 */
...
/** Prefer dedicated Banban kindergarten stills from Game*Spark / Denfaminicogamer. */
const MAP_SOURCES = [
  "https://www.gamespark.jp/imgs/zoom/1163793.jpg",
  "https://www.gamespark.jp/imgs/ogp_f/1163793.jpg",
  "https://news.denfaminicogamer.jp/wp-content/uploads/2026/07/HOEnEtDbAAAAzdW.jpeg",
];

/** Distinct crops so each spot file has unique bytes (no MD5 collisions). */
const SPOT_CROPS = {
  "banban-kindergarten-mural": { left: 80, top: 40, width: 520, height: 380 },
  "banban-welcome-signage": { left: 420, top: 60, width: 500, height: 400 },
  "banban-mascot-prop-cluster": { left: 160, top: 180, width: 540, height: 360 },
  "banban-hallway-poster-wall": { left: 300, top: 120, width: 480, height: 340 },
  "banban-color-theme-wall": { left: 520, top: 200, width: 440, height: 320 },
  "banban-classroom-clutter": { left: 40, top: 220, width: 460, height: 340 },
  // Extra Greece week-two spots cropped from existing greece.jpg
...
const mapDest = path.join(MAP_DIR, "garten-of-banban.jpg");
...
async function downloadMapHero() {
  if (fs.existsSync(mapDest)) {
    ...
    if (
      owner === `maps/garten-of-banban.jpg` ||
      !hashes.has(h) ||
      owner?.endsWith("garten-of-banban.jpg")
    ) {
      console.log(
        `⏭ maps/garten-of-banban.jpg already present (${Math.round(existing.length / 1024)} KB) — skip download`,
      );
      skipped++;
      return;
    }
  ...
  for (const url of MAP_SOURCES) {
    try {
      ...
      assertUnique(hashes, jpg, "maps/garten-of-banban.jpg");
      fs.writeFileSync(mapDest, jpg);
      console.log(`✓ maps/garten-of-banban.jpg (${Math.round(jpg.length / 1024)} KB)`);
      ok++;
      return;
    } catch (e) {
      lastErr = e;
  ...
  throw lastErr ?? new Error("No Banban map source worked");
}

try {
  await downloadMapHero();
} catch (e) {
  console.error(`✗ maps/garten-of-banban: ${e.message}`);
  fail++;
}
